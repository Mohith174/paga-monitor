#!/usr/bin/env python3
"""
SQLite database with analytics for PAGA leads
"""
...
class Database:
    """SQLite database manager with analytics"""
    
    def __init__(self, db_path='paga_leads.db'):
    ...
    def process_case(self, case: Dict) -> Tuple[str, int]:
        ...
        with self.get_conn() as conn:
            ...
            existing = conn.execute(
                "SELECT id, content_hash, version FROM cases WHERE lwda_number = ?",
                (case['lwda_number'],)
            ).fetchone()
            ...
            if not existing:
                ...
                conn.execute("""
                    UPDATE cases SET
                        submission_name=?, submission_type=?, submission_date=?,
                        employer_name=?, employer_city=?, employer_zip=?, num_employees=?,
                        pdf_url=?, content_hash=?, version=?, last_seen=?, updated_at=?,
                        status='new'
                    WHERE lwda_number=?
                """, (
                ...
                # DUPLICATE
                conn.execute(
                    "UPDATE cases SET last_seen=? WHERE lwda_number=?",
    ...
    def get_dashboard_stats(self) -> Dict:
        ...
        with self.get_conn() as conn:
            ...
            stats['new_today'] = conn.execute(
                "SELECT COUNT(*) FROM cases WHERE DATE(submission_date) = ?", (today,)
            ).fetchone()[0]
            
            # Cases filed this week
            stats['new_week'] = conn.execute(
                "SELECT COUNT(*) FROM cases WHERE DATE(submission_date) >= ?", (week_ago,)
            ).fetchone()[0]
            
            # Cases filed this month
            stats['new_month'] = conn.execute(
                "SELECT COUNT(*) FROM cases WHERE DATE(submission_date) >= ?", (month_ago,)
            ).fetchone()[0]
            ...
            avg_response = conn.execute("""
                SELECT AVG((JULIANDAY(contacted_date) - JULIANDAY(first_seen)) * 24)
    ...
    def get_recent_cases(self, limit=50, status=None, min_priority=0) -> List[Dict]:
        ...
        with self.get_conn() as conn:
            ...
            query += " ORDER BY submission_date DESC, first_seen DESC LIMIT ?"
            params.append(limit)
    ...
    def get_case_analysis(self, case_id: int) -> Optional[Dict]:
        ...
        with self.get_conn() as conn:
            row = conn.execute(
                "SELECT * FROM case_analysis WHERE case_id = ?", 
                (case_id,)
            ).fetchone()
            if row:
                d = dict(row)
    ...
    def add_analysis(self, case_id: int, analysis: Dict):
        ...
        with self.get_conn() as conn:
            ...
            if analysis.get('agent_score', 0) > 50:
                conn.execute(
                    "UPDATE cases SET priority = 10, status = 'qualified' WHERE id = ?", 
                    (case_id,)
                )
